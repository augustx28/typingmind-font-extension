(function () {
    ...
    // --- Configuration from Font Button ---
    const SCRIPT_PREFIX = 'tmfc_'; // Prefix for CSS classes and localStorage keys from font script
    ...
    const settingsKeys = {
        ...
        fontChangerCurrentFont: SCRIPT_PREFIX + 'currentFont',
    };
    ...
    // --- State & Storage Helper (from Font Button, slightly adapted) ---
    const storage = {
    ...
    let currentFontSetting = storage.getItem(settingsKeys.fontChangerCurrentFont, { name: '', isGoogle: false, custom: false, weight: DEFAULT_FONT_WEIGHT });
    ...
    // --- Font Changer Core Functions (Integrated) ---
    ...
    function loadGoogleFont(fontName) {
        ...
        link.rel = 'stylesheet';
        link.onerror = () => {
            console.error(`${consolePrefix}Error loading Google Font: ${fontName}.`);
        };
    ...
    function applyFont(fontName, isGoogleFont = false, isCustom = false, fontWeight = DEFAULT_FONT_WEIGHT) {
        ...
        storage.setItem(settingsKeys.fontChangerCurrentFont, currentFontSetting);
    }
    ...
    function createSettingsModal() {
        ...
        const styles = `
            ...
            /* Styles for Font Changer Section */
            .${SCRIPT_PREFIX}form-group { margin-bottom: 18px; }
            .${SCRIPT_PREFIX}form-group label { display: block; margin-bottom: 8px; font-weight: 500; font-size: 0.9em; color: #ccc; }
            ...
            .${SCRIPT_PREFIX}button-group { display: flex; justify-content: flex-end; margin-top: 20px; gap: 12px; }
            .${SCRIPT_PREFIX}button-group button { padding: 8px 16px; border: 1px solid transparent; border-radius: 6px; cursor: pointer; font-size: 0.9em; font-weight: 500; }
            .apply-button { background-color: #0d6efd; color: white; }
        ...
        fontSectionTitle.textContent = "Global Font Customization";
        ...
        fontSettingsContainer.innerHTML += `
            ...
            <div class="${SCRIPT_PREFIX}button-group">
                <button class="apply-button" id="${SCRIPT_PREFIX}apply-font-button">Apply Font</button>
                <button class="reset-button" id="${SCRIPT_PREFIX}reset-font-button">Reset Font</button>
            </div>
        `;
        ...
        // --- Event Listeners for Font Changer ---
        const googleFontSelect = modalElement.querySelector(`#${SCRIPT_PREFIX}google-font-select`);
        const localFontInput = modalElement.querySelector(`#${SCRIPT_PREFIX}local-font-input`);
        const fontWeightSelect = modalElement.querySelector(`#${SCRIPT_PREFIX}font-weight-select`);

        modalElement.querySelector(`#${SCRIPT_PREFIX}apply-font-button`).addEventListener('click', () => {
        ...
        modalElement.querySelector(`#${SCRIPT_PREFIX}reset-font-button`).addEventListener('click', () => {
            applyFont(null);
    ...
    function saveSetting(key, value) {
        try {
            let valueToStore = value;
             if ([settingsKeys.customPageTitle, settingsKeys.customFaviconData].includes(key)) {
            ...
            if (valueToStore === null) {
                localStorage.removeItem(key);
                 ...
                 localStorage.setItem(key, (typeof valueToStore === 'boolean') ? JSON.stringify(valueToStore) : valueToStore);
            }
    ...
    function initializeTweaks() {
        ...
        // Apply saved font on initialization
        if (currentFontSetting.name) {
