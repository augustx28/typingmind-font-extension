...
Original by ITCON, AU | Edited by Enjoy | Optimized for Google Drive Only
-------------------------
Features:
- Google Drive Sync only (S3 removed)
...
if (window.typingMindCloudSync) {
  ...
  class ConfigManager {
    ...
    loadConfig() {
      const defaults = {
        storageType: "googleDrive", // Forced default
        syncInterval: 15,
        encryptionKey: "",
        googleClientId: "",
      };
      ...
      const keyMap = {
        syncInterval: "tcs_aws_syncinterval", // Keeping legacy key name for compatibility
  ...
  class DataService {
    ...
    async *streamAllItems() {
      // Wrapper for internal stream
      yield* this.streamAllItemsInternal();
    }
