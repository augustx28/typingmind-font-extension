(function () {
  ...
  function createSettingsModal() {
    ...
    const styles = `
      ...
      #tweak-modal { background-color: #252525; color: #f0f0f0; padding: 25px 35px; border-radius: 8px; min-width: 420px; max-width: 550px; box-shadow: 0 8px 25px rgba(0,0,0,0.6); position: relative; border: 1px solid #4a4a4a; }
      ...
      .tweak-color-item label { margin-right: 10px; color: #e0e0e0; font-size: 1em; flex-shrink: 0; }
      ...
      .tweak-text-item { margin-top: 15px; padding-top: 15px; border-top: 1px solid #4a4a4a; display: flex; align-items: center; justify-content: space-between; }
      .tweak-text-item.no-border { border-top: none; padding-top: 0; margin-top: 8px; }
      .tweak-text-item label { color: #e0e0e0; font-size: 1em; white-space: nowrap; margin-right: 15px; flex-shrink: 0; min-width: 120px; }
      .tweak-text-input-wrapper { display: flex; align-items: center; flex: 1; justify-content: flex-end; }
      .tweak-text-item input[type='text'], .tweak-text-item input[type='number'] { width: 150px; padding: 6px 10px; border: 1px solid #777; margin-right: 10px; border-radius: 4px; background-color: #555; color: #f0f0f0; font-size: 0.9em; }
      .tweak-text-item input[type='text']::placeholder, .tweak-text-item input[type='number']::placeholder { color: #aaa; opacity: 1; }
      .tweak-text-item input[type='number'] { width: 80px; }
      ...
      .font-customization-subsection-title { color: #d0d0d0; font-size: 0.95em; margin-top: 20px; margin-bottom: 10px; padding-bottom: 5px; border-bottom: 1px dashed #555; }
      /* --- Global Font Section Styles --- */
      ...
      .${SCRIPT_PREFIX}form-group small { display: block; margin-top: 6px; color: #bbbbbb; font-size: 0.85em; }
      ...
      /* --- Favicon Section Styles --- */
      .tweak-favicon-group { display: flex; align-items: center; margin-bottom: 12px; }
      .tweak-favicon-group label { color: #e0e0e0; font-size: 1em; margin-right: 15px; flex-shrink: 0; min-width: 120px; }
      .tweak-favicon-group input[type='file'] { color: #f0f0f0; padding: 5px; border: 1px solid #777; border-radius: 4px; background-color: #555; flex: 1; max-width: 200px; }
      .tweak-favicon-controls { display: flex; align-items: center; min-height: 30px; gap: 10px; }
      .tweak-favicon-controls img { width: 24px; height: 24px; border: 1px solid #777; border-radius: 3px; }
    `;
    ...
    // --- Text Input Helper ---
    function createTextInput(id, labelText, settingKey, placeholder, type = "text", attributes = {}, noBorder = false) {
      const item = document.createElement("div");
      item.className = noBorder ? "tweak-text-item no-border" : "tweak-text-item";
      const label = document.createElement("label");
      label.htmlFor = id;
      label.textContent = labelText;
      const wrapper = document.createElement("div");
    ...
    globalFontSettingsSection.innerHTML = `
      ...
      <div class="${SCRIPT_PREFIX}form-group">
        ...
        <small>Paste a CSS URL. Then provide the exact font-family name below.</small>
      </div>
      <div class="${SCRIPT_PREFIX}form-group">
    ...
    // --- Chat Font Customization Section ---
    ...
    fontScopeNotice.style.cssText = "font-size: 0.85em; color: #bbb; margin-bottom: 15px;";
    ...
    localFontDescription.style.cssText = "margin-bottom: 10px; font-size: 0.9em; color: #ccc;";
    fontSettingsContainer.appendChild(localFontDescription);

    const localFontFamilyInput = createTextInput("tweak_localFontFamily_input", "Local Font:", settingsKeys.localFontFamily, "e.g., Arial, Verdana", "text", {}, true);
    ...
    fontUrlDescription.style.cssText = "margin-bottom: 10px; font-size: 0.9em; color: #ccc;";
    fontSettingsContainer.appendChild(fontUrlDescription);

    const customFontUrlInput = createTextInput("tweak_customFontUrl_input", "Font URL:", settingsKeys.customFontUrl, "Google Fonts URL", "text", {}, true);
    fontSettingsContainer.appendChild(customFontUrlInput);

    const customFontFamilyInput = createTextInput("tweak_customFontFamily_input", "URL Font Family:", settingsKeys.customFontFamily, "Font Name from URL", "text", {}, true);
    ...
    const fontSizeInput = createTextInput("tweak_customFontSize_input", "Font Size (px):", settingsKeys.customFontSize, "e.g., 16", "number", { min: "8", step: "1" }, true);
    ...
    faviconInputGroup.className = "tweak-favicon-group";
    ...
    const handleApplyGlobalFont = () => {
      ...
      if (selectedGoogleFont) {
        ...
        if (!(enteredUrl.startsWith("http://") || enteredUrl.startsWith("https://"))) {
          if (feedbackElement) {
            feedbackElement.textContent = "Invalid Web Font URL. Must start with http:// or https://";
            setTimeout(() => {
              if (feedbackElement.textContent.startsWith("Invalid Web Font URL")) feedbackElement.textContent = " ";
            }, 2500);
          }
          return;
        }
        if (!enteredUrlFamily) {
          if (feedbackElement) {
            feedbackElement.textContent = "Please provide the URL Font Family name.";
            setTimeout(() => {
              if (feedbackElement.textContent.startsWith("Please provide")) feedbackElement.textContent = " ";
            }, 2500);
          }
          return;
        }
        settingToSave = {
          name: enteredUrlFamily,
          isGoogle: false,
          isUrl: true,
          custom: false,
          url: enteredUrl,
        ...
        if (currentSetting && (currentSetting.name || currentSetting.isUrl)) {
          ...
          if (feedbackElement) {
            feedbackElement.textContent = "No Global UI Font selected.";
            setTimeout(() => {
              if (feedbackElement.textContent === "No Global UI Font selected.") feedbackElement.textContent = " ";
            }, 2000);
          }
          return;
        }
    ...
    // Mutual exclusivity for font inputs
  ...
  function saveSetting(key, value) {
    try {
      ...
      if (feedbackElement && key !== settingsKeys.customFaviconData) {
        ...
        setTimeout(() => {
          if (feedbackElement.textContent === "Settings saved.") feedbackElement.textContent = " ";
        }, 2000);
      }
