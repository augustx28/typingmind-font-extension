...
async download(key, isMetadata = false) {
  ...
  class CloudSyncApp {
    ...
    async setupSyncButtonObserver() {
      ...
      // Inject styles if not present (Status Dot + Modal Styles)
      if (!document.getElementById("tcs-inline-style")) {
        ...
        #sync-status-dot {
          ...
          display: none;
          z-index: 10;
          box-shadow: 0 0 0 2px var(--bg-base, #18181b);
        }
      ...
      // Tooltip attributes to match Settings button behavior
      ...
      // Updated Classes: Matches reference button geometry (w-9 h-9, rounded-[10px])
      // Preserved dual-mode colors (slate-900 for light mode, white for dark mode)
      button.className = "text-slate-900/70 sm:hover:bg-slate-900/20 dark:text-white/70 sm:dark:hover:bg-white/20 flex rounded-[10px] w-9 h-9 items-center justify-center shrink-0 transition-colors cursor-default focus:outline-0 relative";
      
      button.style.cursor = "pointer";
      
      // Updated InnerHTML: Icon only (18x18) + Status Dot. No text span.
      button.innerHTML = `
