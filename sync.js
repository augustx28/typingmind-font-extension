...
async download(key, isMetadata = false) {
  ...
  class CloudSyncApp {
    ...
    async setupSyncButtonObserver() {
      ...
      // 2. Locate the anchor (Chat) and the reference model (Settings)
      ...
      // We need the settings button to copy styles from. If not found yet, retry later.
      if (!chatButton?.parentNode || !settingsButton)
        return false;

      // 3. Inject CSS for the status dot and modal styling
      if (!document.getElementById("tcs-inline-style")) {
        ...
        #sync-status-dot {
          ...
          display: none;
          z-index: 10;
          box-shadow: 0 0 0 2px rgba(39, 39, 42, 1);
        }
        ...
        /* Modal form controls */
        .cloud-sync-modal input, .cloud-sync-modal select, .cloud-sync-modal textarea {
          background-color: #3f3f46 !important; color: #ffffff !important; border-color: #52525b !important;
        }
        .cloud-sync-modal input::placeholder { color: #a1a1aa !important; }
        .cloud-sync-modal label { color: #d4d4d8 !important; }
        `;
      ...
      // 5. Define Inner HTML 
      // We wrap the SVG in a div to hold the status dot relative to the icon.
      // The text span is separate so we can hide it when minimized.
      button.innerHTML = `
        <div class="relative flex items-center justify-center pointer-events-none">
          <svg class="w-[18px] h-[18px] flex-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18">
        ...
        <span id="sync-text-label" class="font-semibold mx-auto text-center text-xs leading-4 md:leading-none w-full md:w-[51px]" style="hyphens: auto; word-break: break-word; display: none;">Sync</span>
      `;

      // 6. Function to mirror the Settings button classes dynamically
      const mirrorStyles = () => {
          // Copy all classes from the Settings button to ensure exact alignment
          button.className = settingsButton.className;
          
          // Detect state: The expanded/mobile button uses 'flex-col' to stack Icon + Text.
          // The minimized/pinned button does NOT use 'flex-col' (it centers the icon).
          const isExpanded = button.classList.contains('flex-col');
          
          // Toggle text label visibility based on state
          const label = button.querySelector('#sync-text-label');
          if (label) {
              label.style.display = isExpanded ? 'block' : 'none';
              // Add a small margin top if expanded to match the 'gap-1.5' usually found in flex-col
              label.style.marginTop = isExpanded ? '4px' : '0';
          }

          // Mirror Tooltip attributes (TypingMind adds these when minimized)
          if (settingsButton.hasAttribute('data-tooltip-content')) {
              button.setAttribute('data-tooltip-content', 'Cloud Sync');
              button.setAttribute('data-tooltip-id', settingsButton.getAttribute('data-tooltip-id'));
              button.setAttribute('data-tooltip-place', settingsButton.getAttribute('data-tooltip-place'));
          } else {
              button.removeAttribute('data-tooltip-content');
              button.removeAttribute('data-tooltip-id');
              button.removeAttribute('data-tooltip-place');
          }
      };

      // 7. Initialize styles immediately
      mirrorStyles();

      // 8. Watch the Settings button for class changes (e.g. when user collapses sidebar)
      // This ensures the Sync button updates instantly to match.
      const observer = new MutationObserver(() => mirrorStyles());
      observer.observe(settingsButton, { attributes: true, attributeFilter: ['class'] });

      // 9. Add click listener and insert into DOM
