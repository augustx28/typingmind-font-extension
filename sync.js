/*TypingMind Cloud Sync v4.2 - Google Drive Only
Based on original by ITCON, AU
Modified by Enjoy for attachment support
Cleaned for Google Drive only usage
-------------------------
Features:
...
if (window.typingMindCloudSync) {
  ...
  class SyncOrchestrator {
        ...
        const uploadPromises = changedItems.map(async (item) => {
          ...
          try {
            if (item.deleted || item.reason === "tombstone") {
              ...
              const mime =
                item.type === "blob" && data instanceof Blob
                  ? data.type
                  : item.blobType || "";
              ...
              if (data) {
                ...
                if (item.type === "blob") {
                  ...
                  newMetadataEntry.size = data.length || item.size || 0;
    ...
    async syncFromCloud() {
      ...
      try {
        ...
        if (purgedCount > 0) {
          this.logger.log(
            "warning",
            `Purged ${purgedCount} newly excluded item(s) from cloud metadata to resolve conflicts.`
          );
        ...
        const cloudActiveCount = Object.values(
          cloudMetadata.items || {}
        ).filter((item) => !item.deleted).length;
        const localActiveCount = Object.values(
          this.metadata.items || {}
        ).filter((item) => !item.deleted).length;

        if (
          !hasCloudChanges &&
          cloudActiveCount === localActiveCount &&
          !metadataWasPurged
        ) {
          this.logger.log(
            "info",
    ...
    async performFullSync() {
      ...
      if (metadataWasPurged) {
        this.logger.log(
          "info",
          "Metadata was purged. Forcing an upload to make the fix permanent in the cloud."
        );
    ...
    async _getBackupIndex() {
      try {
        ...
        if (error.message && error.message.includes("Invalid JSON")) {
          this.logger.log(
            "warn",
            "Backup index is corrupted, will rebuild it."
          );
          try {
            ...
            this.logger.log(
              "warn",
              "Could not delete corrupted index",
              deleteError
            );
          }
          return null;
  ...
  class CloudSyncApp {
    constructor() {
      this.footerHTML =
        '<span style="color:rgb(197, 192, 192);">Based on original by ITCON, AU</span> <br><a href="https://github.com/itcon-pty-au/typingmind-cloud-backup" target="_blank" rel="noopener noreferrer" style="color:rgb(197, 192, 192);">Github</a>';
    ...
    async initialize() {
      this.logger.log("start", "Initializing TypingMind Cloud Sync (Google Drive Only)");
      ...
      const storageType = "googleDrive";
      this.logger.log("info", `Using storage provider: Google Drive`);
      ...
      try {
        ...
        if (ProviderClass) {
          ...
          throw new Error(`Storage provider not available: '${storageType}'`);
        }
      } catch (error) {
        this.logger.log(
          "error",
      ...
      if (this.noSyncMode) {
        ...
        if (this.storageService.isConfigured()) {
          ...
          if (!this.checkMandatoryConfig()) {
            alert(
              "⚠️ Cloud Sync Configuration Required\n\nPlease click the Sync button to open settings and configure Google Drive, then reload the page."
            );
          }
        }
      }
      ...
      return `<div class="text-white text-left text-sm">
        ...
        <div class="cloud-sync-modal-header">
          <div class="flex justify-between items-center gap-3">
            <h3 class="text-xl font-bold text-white">Cloud Sync (Google Drive)</h3>
            <div class="flex items-center gap-2">
              <span class="text-sm text-zinc-400">Auto-Sync</span>
              <input type="checkbox" id="auto-sync-toggle" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded cursor-pointer" ${
        ...
        <div class="cloud-sync-modal-content">
            ...
            <div id="available-backups-content" class="space-y-2 hidden">
              <div class="w-full">
                <select id="backup-files" class="w-full px-2 py-1.5 border border-zinc-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700 text-white">
                  <option value="">Please configure Google Drive first</option>
                </select>
              </div>
          ...
          <div class="mt-4 bg-zinc-800 px-3 py-2 rounded-lg border border-zinc-700">
            <div class="flex items-center justify-between mb-2 cursor-pointer select-none" id="provider-settings-header">
              <label class="block text-sm font-medium text-zinc-300">Google Drive Settings</label>
              <div class="flex items-center gap-1">
    ...
    setupModalEventListeners(modal, overlay) {
      ...
      // Directly inject Google Drive UI since it's the only provider
      const { html, setupEventListeners } =
        GoogleDriveService.getConfigurationUI();
    ...
    async loadSyncDiagnostics(modal) {
      ...
      if (!this.storageService || !this.storageService.isConfigured()) {
        setContent(
          '<tr><td colspan="2" class="text-center py-2 text-zinc-500">Google Drive Not Configured</td></tr>'
        );
    ...
    async saveSettings(modal) {
      ...
      if (!newConfig.encryptionKey) {
        alert("Encryption key is a mandatory setting.");
        return;
      }
      if (!newConfig.googleClientId) {
      ...
      actionMsg.textContent = "Verifying Google Drive credentials...";
    ...
    async createSnapshot() {
      if (!this.isSnapshotAvailable()) {
        alert(
          "⚠️ Snapshot Unavailable\n\nPlease configure and save your Google Drive settings first."
        );
        return;
      }
    ...
    async runLeaderTasks() {
      if (!this.noSyncMode && this.storageService.isConfigured()) {
        if (!this.autoSyncEnabled) {
          this.logger.log(
            "info",
            "Auto-sync is disabled, skipping initial sync on load"
          );
          return;
        }
    ...
    async loadTombstoneList(modal) {
      ...
      if (!this.storageService || !this.storageService.isConfigured()) {
        tableBody.innerHTML =
          '<tr><td colspan="4" class="p-4 text-center text-zinc-500">Google Drive Not Configured</td></tr>';
        return;
      }
  ...
  // Only register Google Drive provider
