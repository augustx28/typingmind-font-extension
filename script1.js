(function () {
  ...
  const defaultSidebarMenuColor = "#18181b"; // Added
  ...
  function applyStylesBasedOnSettings() {
    ...
    const sidebarMenuColor = getSetting(settingsKeys.sidebarMenuColor, null); // Added
    ...
    if (workspaceBar) {
      ...
      // Apply sidebar menu color
      if (sidebarMenuColor) { // Added
    ...
    // Add/update style tag for sidebar menu color // Added
    ...
    sidebarStyle.textContent = `div[data-element-id="workspace-bar"] { background: var(--sidebar-menu-color, ${defaultSidebarMenuColor}) !important; }`;
  }
  ...
  function createSettingsModal() {
    ...
    const sidebarMenuColorPicker = createColorPicker("tweak_sidebarMenuColor_input", "Sidebar Color:", settingsKeys.sidebarMenuColor, defaultSidebarMenuColor); // Added

    function createTextInput(id, labelText, settingKey, placeholder, type = "text", attributes = {}) {
        const item = document.createElement("div");
        item.className = "tweak-text-item";
        if (type !== "number") { // Adjusted logic slightly from original for consistency, title input needs border-top.
             // This condition might need adjustment if more text inputs are added before others that need a top border.
             // For now, relying on specific styling below for title input.
        }
        const label = document.createElement("label");
        label.htmlFor = id;
        label.textContent = labelText;
    ...
    customTitleInput.style.borderTop = "1px solid #4a4a4a"; // Ensure title input has top border
    ...
    // Appending all sections to scrollable content
    scrollableContent.append(
        settingsSection,
        newChatColorPicker,
        wsIconColorPicker,
        wsFontColorPicker,
        sidebarMenuColorPicker, // Added here
        customTitleInput,
  ...
  function loadSettingsIntoModal() {
    ...
    document.getElementById("tweak_sidebarMenuColor_input").value = getSetting(settingsKeys.sidebarMenuColor, defaultSidebarMenuColor); // Added
  ...
  function saveSetting(key, value) {
    try {
      ...
      if ([settingsKeys.customFontUrl, settingsKeys.customFontFamily, settingsKeys.localFontFamily, settingsKeys.customPageTitle, settingsKeys.customFaviconData, settingsKeys.sidebarMenuColor].includes(key)) { // Added sidebarMenuColor here for string null check
