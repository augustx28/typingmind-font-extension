(function () {
    ...
    const handleApplyGlobalFont = () => {
      ...
      if (selectedGoogleFont) {
        ...
        if (!(enteredUrl.startsWith("http://") || enteredUrl.startsWith("https://"))) {
          if (feedbackElement) {
            feedbackElement.textContent = "Invalid Web Font URL. It must start with http:// or https://";
            setTimeout(() => {
        ...
        if (!enteredUrlFamily) {
          if (feedbackElement) {
            ...
            setTimeout(() => {
              if (feedbackElement.textContent.startsWith("Please provide the URL Font Family")) feedbackElement.textContent = " ";
            }, 2500);
          }
          return;
        }
  ...
  function loadSettingsIntoModal() {
    ...
    const wsIconInput = document.getElementById("tweak_workspaceIconColor_input");
    if (wsIconInput) {
      wsIconInput.value = getSetting(settingsKeys.workspaceIconColor, defaultWorkspaceIconColorVisual);
    }
  ...
  const observer = new MutationObserver(() => {
    ...
    if (workspaceBar) {
      ...
      // We prioritize buttons that are less likely to be "active" (selected) to avoid copying active styles.
      // Priority: Cloud Sync -> Settings -> Profile
      ...
      const profileBtn = document.querySelector('button[data-element-id="workspace-profile-button"]') ? document.querySelector('button[data-element-id="workspace-profile-button"]').parentElement : null;

      // Select the first available button from our priority list
      const referenceButtonForStyle = cloudSyncBtn || settingsBtn || profileBtn;

      if (!referenceButtonForStyle) return;

      // 1. Create button if it doesn't exist
      if (!tweaksButton) {
        ...
        textSpan.className = "font-semibold mx-auto self-stretch text-center text-xs leading-4 md:leading-none w-full md:w-[51px]";
        textSpan.style.hyphens = "auto";
        textSpan.style.wordBreak = "break-word";
        textSpan.textContent = "Tweaks";

        tweaksButton.appendChild(svgIcon);
        tweaksButton.appendChild(textSpan);
      ...
      // 2. CONTINUOUS SYNC: Ensure class and text visibility matches reference button
      if (tweaksButton && referenceButtonForStyle) {
        let targetClass = referenceButtonForStyle.className;

        // --- STYLE NORMALIZATION ---
        // If the reference button is "Active" (e.g., Settings is open), it will have active classes (bg-white/20, text-white).
        // If our Tweaks modal is closed, we must NOT look active. We downgrade the classes.
        const isTweaksActive = modalOverlay && window.getComputedStyle(modalOverlay).display !== "none";
        
        // Detect if reference is active (Common heuristic: has background but not hover-only background)
        const isRefActive = targetClass.includes("bg-white/20") && !targetClass.includes("hover:bg-white/20");

        if (isRefActive && !isTweaksActive) {
          // Force Inactive Look:
          // 1. Remove active background (bg-white/20)
          // 2. Add hover background (sm:hover:bg-white/20) if missing
          // 3. Dim text (text-white -> text-white/70)
          ...
          targetClass = targetClass.replace(/\btext-white\b/g, "text-white/70");
           ...
           // Force Active Look (If Tweaks is open but reference is closed):
        ...
        // Sync Text Visibility (Hide "Tweaks" text if "Settings" text is hidden/missing)
        const refSpan = referenceButtonForStyle.querySelector("span");
        const mySpan = tweaksButton.querySelector("span");
        if (mySpan) {
          const shouldShowText = refSpan && window.getComputedStyle(refSpan).display !== "none";
          mySpan.style.display = shouldShowText ? "" : "none";
        }

        // Sync Tooltip Behavior
        if (referenceButtonForStyle.hasAttribute("data-tooltip-content")) {
