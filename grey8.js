...
(function() {
  ...
  // Replace the icon with custom SVG (Claude's chat icon)
  function replaceIcon() {
    // Try multiple possible selectors
    const selectors = [
      '.bg-\\[--workspace-color\\] .overflow-hidden .flex-shrink-0',
      '.bg-\\[--workspace-color\\] .justify-center > .overflow-hidden .flex-shrink-0',
      '.overflow-hidden .flex-shrink-0 svg'
    ];
    
    let iconContainer = null;
    for (const selector of selectors) {
      iconContainer = document.querySelector(selector);
    ...
    if (iconContainer && !iconContainer.dataset.iconReplaced) {
      ...
      // Create new SVG element (Claude's chat icon)
  ...
  function observeChanges() {
    const observer = new MutationObserver((mutations) => {
      // Check if we need to replace icon on any changes
