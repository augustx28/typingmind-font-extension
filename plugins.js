/* ============================================================================
 * TypingMind – Mobile-Ready Plugin Sorter & State Manager (V4: Folder Safe)
 * ========================================================================== */
(() => {
    const CONFIG = {
        key_order: 'tm_plugin_sort_v4_order',    // Stores item order
        key_state: 'tm_plugin_sort_v4_toggles',  // Stores Open/Closed folder state
        selectors: {
            // The clickable headers that open/close folders
            header: 'button[id^="headlessui-disclosure-button"]', 
            // The draggable items
            row: '[role="menuitem"]',
            // The text label inside an item
            name: '.truncate',
            // The specific container holding the items (auto-detected now)
            list_container: null 
        }
    };
    ...
    /* --- MODULE 1: FOLDER STATE KEEPER (Remembers Open/Closed) --- */
    class StateKeeper {
        constructor() {
            ...
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((m) => {
                    m.addedNodes.forEach((n) => {
                        if (n.nodeType === 1) {
                            // Find all headers in the new content
        ...
        initHeader(header) {
            ...
            // 1. Identify the header (e.g., "Model tools", "MCP")
            const title = header.textContent.trim();
            if (!title) return;

            // 2. Restore State
            const savedState = this.store.get()[title]; 
            // savedState: true (Open), false (Closed), undefined (Default)
            
            const isOpen = header.getAttribute('aria-expanded') === 'true';
            
            // If we have a saved preference and it differs from current, toggle it.
            ...
            header.addEventListener('click', () => {
                // Wait slightly for aria-expanded to update
                setTimeout(() => {
    ...
    class PluginSorter {
        ...
        injectStyles() {
            if (document.getElementById('tm-v4-css')) return;
        ...
        startObserver() {
            // We watch for items appearing, then find their PARENT list.
            // This ensures we sort the correct sub-folder, not the whole page.
            new MutationObserver((mutations) => {
                mutations.forEach(m => {
                    m.addedNodes.forEach(n => {
                        if (n.nodeType !== 1) return;
                        
                        // Find any plugin items inside this new node
                        const items = n.querySelectorAll ? n.querySelectorAll(CONFIG.selectors.row) : [];
                        if (items.length === 0) return;

                        // Identify the unique parents (lists) of these items
                        const lists = new Set();
                        items.forEach(item => {
        ...
        initList(list) {
            ...
            // 1. Add Handles to existing items
            ...
            // 3. Watch this specific list for new items (like when toggling "Show All")
            new MutationObserver(() => {
                const newRows = list.querySelectorAll(`:scope > ${CONFIG.selectors.row}`);
                newRows.forEach(r => this.addHandle(r));
        ...
        dragStart(e, row) {
            e.preventDefault();
            const list = row.parentElement; // STRICT PARENT
        ...
        dragMove(e) {
            ...
            // We scroll the closest scrollable ancestor
        ...
        autoScroll(y) {
            // Find scroll parent dynamically or use the main one
        ...
        checkSort(y) {
            ...
            // Only compare with siblings in the SAME list
        ...
        saveOrder(list) {
            // We save a master list of all known items -> index
            // Or simpler: Save the order array for this specific list context? 
            // Since items might move between folders in future updates, 
            // let's just save a global "Priority List" of names.
            
            // Get current global order from storage to merge
            let globalOrder = this.orderStore.get().order || [];
            
            // Get names in this current list
            ...
            // Remove these names from global order then re-insert them in new sequence
            globalOrder = globalOrder.filter(n => !currentNames.includes(n));
            globalOrder.push(...currentNames); // Append new order (or handle complex merge if needed)
            
            // Note: A flat list is easiest. If "Hugging Face" is at end of array, it goes to bottom of its container.
        ...
        applySavedOrder(list) {
            ...
            // Re-append in order of the global list
            // Items NOT in the global list will remain at the top (or bottom depending on impl)
            // Strategy: Loop global order, if item exists in this list, append it.
