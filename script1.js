(function () {
  ...
  const settingsKeys = {
    ...
    sidebarMenuColor: "tweak_sidebarMenuColor", // Added
  ...
  const defaultSidebarMenuColor = "#18181b"; // Added: Default from script1.js for sidebar
  ...
  function applyStylesBasedOnSettings() {
    ...
    const sidebarMenuColorValue = getSetting(settingsKeys.sidebarMenuColor, null); // Renamed for clarity within this function
    ...
    if (workspaceBar) {
      ...
      // Apply sidebar menu color to workspaceBar via CSS variable
      if (sidebarMenuColorValue) {
        workspaceBar.style.setProperty("--sidebar-menu-color", sidebarMenuColorValue);
      } else {
    ...
    // Inject/Update the style rule that uses the --sidebar-menu-color CSS variable
    let sidebarStyleElement = document.getElementById("tweak-sidebar-menu-color-style");
    if (!sidebarStyleElement) {
      sidebarStyleElement = document.createElement("style");
      sidebarStyleElement.id = "tweak-sidebar-menu-color-style";
      document.head.appendChild(sidebarStyleElement);
    }
    // The CSS rule applies the background to the workspace-bar.
    // The fallback color in var() is defaultSidebarMenuColor.
    sidebarStyleElement.textContent = `div[data-element-id="workspace-bar"] { background: var(--sidebar-menu-color, ${defaultSidebarMenuColor}) !important; }`;
  }
  ...
  function createSettingsModal() {
    ...
    function createColorPicker(id, labelText, settingKey, defaultValue) {
        ...
        resetButton.addEventListener("click", () => {
            saveSetting(settingKey, null); // Save null to remove specific color
            input.value = defaultValue; // Visually reset input to its default
            applyStylesBasedOnSettings(); // Re-apply styles to reflect the reset
        });
    ...
    const sidebarMenuColorPicker = createColorPicker("tweak_sidebarMenuColor_input", "Sidebar Color:", settingsKeys.sidebarMenuColor, defaultSidebarMenuColor);
    ...
    function createTextInput(id, labelText, settingKey, placeholder, type = "text", attributes = {}) {
        ...
        input.addEventListener("input", (e) => saveSetting(settingKey, e.target.value || null)); // Save empty as null
  ...
  function loadSettingsIntoModal() {
    ...
    document.getElementById("tweak_sidebarMenuColor_input").value = getSetting(settingsKeys.sidebarMenuColor, defaultSidebarMenuColor);
    ...
    document.getElementById("tweak_customFontSize_input").value = fontSize !== null ? String(fontSize) : "";
  ...
  function saveSetting(key, value) {
    try {
      ...
      // For string-based settings that can be cleared to null (like colors, URLs, font names)
      if ([
            ...
            settingsKeys.newChatButtonColor, // color can be null
            settingsKeys.workspaceIconColor, // color can be null
            settingsKeys.workspaceFontColor, // color can be null
            settingsKeys.sidebarMenuColor   // color can be null
          ].includes(key)) {
        valueToStore = (value && String(value).trim() !== "" && String(value).trim().toLowerCase() !== "null") ? String(value).trim() : null;
      } else if (key === settingsKeys.customFontSize) {
        valueToStore = (value !== null && String(value).trim() !== "" && !isNaN(parseInt(value,10))) ? parseInt(value,10) : null;
      }
      // For booleans, JSON.stringify handles them, null is not typical unless explicitly desired.

      if (valueToStore === null) {
        localStorage.removeItem(key);
      } else {
        // Booleans and numbers are stringified, direct strings are stored as is.
      ...
      applyStylesBasedOnSettings(); // This is key to reflect changes immediately
  ...
  function initializeTweaks() {
    ...
    createSettingsModal(); // Creates modal structure but doesn't show it
    applyStylesBasedOnSettings(); // Apply all visual tweaks
    applyCustomTitle();
    applyCustomFont();
    applyCustomFavicon();
...
})(); even though I know it's true I can't even exaggerated enough to make the write
