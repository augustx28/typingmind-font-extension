(function () {
  ...
  const settingsKeys = {
    ...
    // showModalButton: "tweak_showModalButton", // This option has been removed.
  ...
  function applyStylesBasedOnSettings() {
    ...
    // --- FIX FOR LOGO HIDING ---
    // Removed specific src check and used more robust container selection
    const logoImage = document.querySelector('img[alt="TypingMind"]');
    if (logoImage) {
        // Try to find the link wrapping the image, then that link's parent div
        const logoLink = logoImage.closest('a');
        if (logoLink && logoLink.parentElement) {
             logoLink.parentElement.style.display = hideLogo ? "none" : "";
        } else if (logoImage.parentElement && logoImage.parentElement.parentElement) {
