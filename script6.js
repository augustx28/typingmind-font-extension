/*  TypingMind Custom-Font Extension
    v1.1.3 | Shift+Alt+F opens the panel
...
#chat-container, #chat-container * {
  ...
  function buildModal() {
    ...
    window.TMFontModal = { toggle() { overlay.style.display = overlay.style.display === "flex" ? "none" : "flex"; } };
  }
  ...
  function addMenuButton() {
    ...
    // **THE FIX**: Copy classes BUT filter out icon-specific ones (e.g., "i-lucide-settings")
    if (settingsButton.className) {
      const baseClasses = [...settingsButton.classList]
        .filter(c => !c.startsWith("i-")) // Remove any "i-..." class like "i-lucide-settings"
        .join(" ");
      fontButton.className = baseClasses;
    }
    ...
    // Robust innerHTML structure for the button
    ...
    fontButton.addEventListener("click", () => {
      if (window.TMFontModal) window.TMFontModal.toggle();
    });
  }
  ...
  /* ---------- observe DOM changes to add menu button if needed ---------- */
  const observer = new MutationObserver(() => {
    if (!document.getElementById("tm-font-menu-button")) {
        addMenuButton();
    }
    // We can disconnect if the button is found and TypingMind's UI is stable,
    // but keeping it active handles some dynamic UI rebuilds.
    // If performance issues arise, consider disconnecting:
    // if (document.getElementById("tm-font-menu-button")) observer.disconnect();
  });
  ...
  console.log(
    "%cTypingMind Font Extension loaded (v1.1.3) – Shift+Alt+F to open settings",
