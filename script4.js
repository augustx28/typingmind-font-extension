...
#chat-container, #chat-container * {
  ...
  function addMenuButton() {
    ...
    // Create SVG icon for better appearance - an "Aa" icon for font/text settings
    const iconSvg = `
    ...
    if (settingsInnerSpan) {
      ...
      const iconContainer = clone.querySelector("div"); // Assuming the icon is in a div
      if (iconContainer) {
        iconContainer.innerHTML = iconSvg;
      }
      
      // Update the text label
      const textSpan = clone.querySelector("span"); // Assuming text is in a nested span
      if (textSpan) {
      ...
      // This structure might need adjustment based on TypingMind's actual button structure
  ...
  document.addEventListener("keydown", (e) => {
    ...
    // Use Alt for non-Mac, Meta (Cmd) for Mac, consistent with common shortcuts
    const modOk = mac ? (e.metaKey && !e.ctrlKey) : e.altKey; 
  ...
  const observer = new MutationObserver(() => {
    // Check if the modal exists and the button doesn't, then try to add.
  ...
  function initializeExtension() {
    buildModal(); // Build modal first so TMFontModal is defined
    addMenuButton(); // Then add button which might rely on TMFontModal
  ...
  console.log(
    "%cTypingMind Font Extension loaded – Shift+Alt+F (Win/Linux) or Shift+Cmd+F (Mac) to open settings",
