(function () {
  ...
  function removeActiveGlobalUIFontLink() {
    ...
    // Also remove any existing link by ID
    const existingLink = document.getElementById('tweak-global-ui-font-link');
    if (existingLink) existingLink.remove();
  }

  function loadGlobalGoogleFont(fontName, callback) {
    ...
    link.href = fontUrl;
    link.rel = 'stylesheet';
    link.id = 'tweak-global-ui-font-link';
    link.onload = () => {
      console.log(`${consolePrefix} Google Font loaded: ${fontName}`);
      if (callback) callback();
    };
    link.onerror = () => {
  ...
  function loadGlobalUrlFont(fontUrl, callback) {
    removeActiveGlobalUIFontLink();
    ...
    link.onload = () => {
      console.log(`${consolePrefix} Web Font URL loaded: ${cleaned}`);
      if (callback) callback();
    };
    link.onerror = () => {
  ...
  // --- FIXED: Apply Global UI Font - EXCLUDES chat area ---
  function applyGlobalUiFont() {
    ...
    const applyStyles = () => {
      ...
      // CSS targeting UI elements ONLY - excluding chat area
      const cssRules = `
        /* Workspace bar and sidebar */
        ...
        [data-element-id="workspace-profile-button"] *,
        [data-element-id="search-chats-button"],
        [data-element-id="search-chats-button"] *,
        
        /* Pinned characters */
        [data-element-id="pinned-characters-container"],
        [data-element-id="pinned-characters-container"] *,
        
        /* Chat input area (not messages) */
        [data-element-id="chat-input-container"],
        [data-element-id="chat-input-container"] *,
        ...
        /* Settings and other panels - but not chat messages */
        .fixed.inset-0,
        .fixed.inset-0 > div:not([data-element-id="chat-space-middle-part"]) *
        {
    ...
    // Load font source then apply styles
    if (fontSetting.isGoogle && fontSetting.name) {
      loadGlobalGoogleFont(fontSetting.name, applyStyles);
    } else if (fontSetting.isUrl && fontSetting.url) {
      loadGlobalUrlFont(fontSetting.url, applyStyles);
    } else {
      // Local font - no external loading needed
      removeActiveGlobalUIFontLink();
      applyStyles();
    }
  }

  // --- FIXED: Apply Custom Chat Font ---
  function applyCustomFont() {
    ...
    const linkId = "tweak-custom-font-link";
    ...
    // Handle font URL loading via link element (more reliable than @import)
    if (cleanedUrl && (cleanedUrl.startsWith("http://") || cleanedUrl.startsWith("https://"))) {
      ...
      if (linkElement.href !== cleanedUrl) {
    ...
    if (styleDeclarations.length > 0) {
      ...
      // More comprehensive selectors for chat area
      const cssRules = `
        ...
        /* Message containers */
        [data-element-id="chat-space-middle-part"] .whitespace-pre-wrap,
        [data-element-id="chat-space-middle-part"] .text-sm,
        [data-element-id="chat-space-middle-part"] .text-base,
        
        /* Response/message text */
        [data-element-id="chat-space-middle-part"] p,
        [data-element-id="chat-space-middle-part"] span,
        [data-element-id="chat-space-middle-part"] li,
        [data-element-id="chat-space-middle-part"] div:not([class*="code"]):not([class*="hljs"]),
        ...
        /* Message groups */
        .group.flex.p-3.gap-3.items-start,
        .group.flex.p-3.gap-3.items-start *
        {
          ${declarationsString}
        }
        ...
        [data-element-id="chat-space-middle-part"] .hljs,
        [data-element-id="chat-space-middle-part"] [class*="code"] {
  ...
  // --- FIXED: Apply Styles Based On Settings ---
  function applyStylesBasedOnSettings() {
    ...
    if (workspaceBar) {
      // --- FIXED: Hide KB button - multiple selector approaches ---
      // Try direct data-element-id first
      let kbButton = workspaceBar.querySelector('button[data-element-id="workspace-tab-kb"]');
      
      // If not found, search by text content
      if (!kbButton) {
        const allButtons = workspaceBar.querySelectorAll("button");
        allButtons.forEach((button) => {
          const spans = button.querySelectorAll("span");
          spans.forEach((span) => {
            if (span.textContent.trim() === "KB" || span.textContent.trim() === "Knowledge Base") {
              kbButton = button;
            }
          });
        });
      ...
      // Also try looking for any element with KB text
      if (!kbButton) {
      ...
      if (kbButton) {
        kbButton.style.display = hideKB ? "none" : "";
      }
      ...
      // Apply font colors to text spans
  ...
  function createSettingsModal() {
    ...
    globalFontSettingsSection.innerHTML = `
        <h3>Global UI Font (Excludes Chat Area)</h3>
        ...
        <div class="${SCRIPT_PREFIX}form-group">
            ...
            <small style="display:block;margin-top:6px;color:#bbbbbb;">Paste a Google Fonts or other CSS URL containing @font-face rules.</small>
        </div>
        <div class="${SCRIPT_PREFIX}form-group">
            <label for="${SCRIPT_PREFIX}global-font-family">URL Font Family Name:</label>
            <input type="text" id="${SCRIPT_PREFIX}global-font-family" placeholder="Exact font-family name (e.g., Roboto)">
            <small style="display:block;margin-top:6px;color:#bbbbbb;">The exact font-family name as defined in the CSS URL above.</small>
        </div>
        <div class="${SCRIPT_PREFIX}form-group">
    ...
    const customFontFamilyInput = createTextInput("tweak_customFontFamily_input", "Font Family Name:", settingsKeys.customFontFamily, "Font Name from URL");
    ...
    // --- Event Listeners for Global Font Controls ---
    ...
    const handleApplyGlobalFont = () => {
      ...
      if (selectedGoogleFont) {
        ...
        if (!(enteredUrl.startsWith("http://") || enteredUrl.startsWith("https://"))) {
          if (feedbackElement) {
            feedbackElement.textContent = "Invalid URL. Must start with http:// or https://";
            setTimeout(() => {
              if (feedbackElement.textContent.startsWith("Invalid URL")) feedbackElement.textContent = " ";
            }, 2500);
          }
          return;
        }
        if (!enteredUrlFamily) {
          if (feedbackElement) {
            feedbackElement.textContent = "Please provide the Font Family Name for the URL.";
            setTimeout(() => {
        ...
        if (currentSetting && (currentSetting.name || currentSetting.isUrl)) {
          settingToSave = { ...currentSetting,
            weight: selectedWeight
          };
        } else {
          if (feedbackElement) {
            feedbackElement.textContent = "No font selected.";
            setTimeout(() => {
              if (feedbackElement.textContent === "No font selected.") feedbackElement.textContent = " ";
            }, 2000);
          }
          return;
        }
  ...
  // MutationObserver with all apply functions
