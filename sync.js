...
async download(key, isMetadata = false) {
  ...
  if (!chatButton?.parentNode) return false;

  // keep your existing inline style injection exactly as you had it
  ...
  const svg = `
    <svg class="w-[18px] h-[18px] flex-shrink-0" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const render = () => {
    ...
    const chatButtonNow = document.querySelector(
      'button[data-element-id="workspace-tab-chat"]'
    );

    // Detect thin pinned mode from Settings button (w-9 h-9 icon-only)
    const isThinPinned =
      !!settingsButton &&
      settingsButton.classList.contains("w-9") &&
      settingsButton.classList.contains("h-9");

    // Copy classes from the real TypingMind buttons so spacing matches perfectly
    const base = isThinPinned ? settingsButton : chatButtonNow;
    if (base?.className) button.className = base.className;

    // Make it clickable even if base has cursor-default
    button.classList.remove("cursor-default");
    button.classList.add("cursor-pointer");

    // Match DOM shape:
    // - thin pinned: icon only
    // - normal: icon + label (like other tabs)
    button.innerHTML = isThinPinned
      ? `
      ...
      : `
        ...
        <span class="font-semibold mx-auto self-stretch text-center text-xs leading-4 md:leading-none w-full md:w-[51px]"
          style="hyphens: auto; word-break: break-word;">Sync</span>
      `;
  };

  // Insert, then render once
  chatButton.parentNode.insertBefore(button, chatButton.nextSibling);
  render();

  // Watch for sidebar mode changes and re-render
  if (!this._tcsSyncBtnObserver) {
    this._tcsSyncBtnObserverState = { watchingSettings: false };

    const setupObserver = () => {
      const settingsBtn = document.querySelector(
        'button[data-element-id="workspace-tab-settings"]'
      );
      const shouldWatchSettings = !!settingsBtn;

      if (this._tcsSyncBtnObserverState.watchingSettings === shouldWatchSettings) {
        return;
      }

      this._tcsSyncBtnObserver?.disconnect();
      this._tcsSyncBtnObserver = new MutationObserver(() => {
        render();
        setupObserver();
      });

      this._tcsSyncBtnObserverState.watchingSettings = shouldWatchSettings;

      if (shouldWatchSettings) {
        this._tcsSyncBtnObserver.observe(settingsBtn, {
          attributes: true,
          attributeFilter: ["class"],
        });
      } else {
        this._tcsSyncBtnObserver.observe(document.body, {
          childList: true,
          subtree: true,
        });
      }
