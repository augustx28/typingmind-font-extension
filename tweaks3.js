(function () {
  ...
  const observer = new MutationObserver(() => {
    ...
    if (workspaceBar) {
      let tweaksButton = document.getElementById("workspace-tab-tweaks");
      // Fallback strategies for reference button
      ...
      // This handles the switch between Expanded and Pinned modes automatically.
      if (tweaksButton && referenceButtonForStyle) {
        // Sync Classes (width, height, flex layout, hover effects)
        if (tweaksButton.className !== referenceButtonForStyle.className) {
        ...
        if (mySpan) {
          // If reference has no span or it's display:none, hide our span
          const shouldShowText = refSpan && window.getComputedStyle(refSpan).display !== "none";
          mySpan.style.display = shouldShowText ? "" : "none";
        }

        // Sync Tooltip Behavior (Pinned mode uses content attribute, Expanded often doesn't)
