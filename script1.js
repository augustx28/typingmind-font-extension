(function () {
  ...
  const settingsKeys = {
    ...
    // --- NEW: Global UI Font Keys (from font button script) ---
    globalUiFont: "tweak_globalUiFont", // Will store an object: { name, isGoogle, custom, weight }
  };
  ...
  let activeGlobalGoogleFontLink = null; // To keep track of the injected global font stylesheet
  ...
  // --- NEW: Function to load a Google Font for the Global UI ---
  function loadGlobalGoogleFont(fontName) {
      if (activeGlobalGoogleFontLink) {
          activeGlobalGoogleFontLink.remove();
          activeGlobalGoogleFontLink = null;
      }
      if (!fontName) return;
      ...
      activeGlobalGoogleFontLink = link;
  }

  // --- NEW: Function to apply the Global UI Font ---
  function applyGlobalUiFont() {
      ...
      if (!fontSetting || !fontSetting.name) { // Resetting
          ...
          } else if (activeGlobalGoogleFontLink) { // Using local font, remove Google Font
              activeGlobalGoogleFontLink.remove();
              activeGlobalGoogleFontLink = null;
          }
          const fontFamilyStack = `"${fontSetting.name}", ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"`;
  ...
  function createSettingsModal() {
    ...
    const handleApplyGlobalFont = () => {
        ...
        if (selectedGoogleFont) {
            settingToSave = { name: selectedGoogleFont, isGoogle: true, custom: false, weight: selectedWeight };
        } else if (enteredLocalFont) {
            settingToSave = { name: enteredLocalFont, isGoogle: false, custom: true, weight: selectedWeight };
        } else {
            // If both are empty, check if a font is already active to just update its weight
  ...
  function loadSettingsIntoModal() {
    ...
    // --- NEW: Load Global Font Settings into Modal ---
    const fontSetting = getSetting(settingsKeys.globalUiFont, null);
    const googleFontSelect = document.getElementById(`${SCRIPT_PREFIX}google-font-select`);
    const localFontInput = document.getElementById(`${SCRIPT_PREFIX}local-font-input`);
    const fontWeightSelect = document.getElementById(`${SCRIPT_PREFIX}font-weight-select`);
  ...
  function saveSetting(key, value) {
    try {
        ...
        if (valueToStore === null) {
            ...
            // For objects or booleans/numbers, stringify. For regular strings, store directly.
        ...
        // --- NEW: Apply global font on change ---
  ...
  function initializeTweaks() {
    ...
    applyGlobalUiFont(); // NEW: Global UI font
    applyCustomFavicon();
