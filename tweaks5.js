(function () {
  ...
  // --- FIXED: Apply the Global UI Font using CSS injection with !important ---
  function applyGlobalUiFont() {
    ...
    const cssRules = `
      ...
      button, span, div, p, input, textarea, select, label, a,
      .prose, .prose *, .text-sm, .text-base, .text-xs, .text-lg {
  ...
  // --- FIXED: Added applyGlobalUiFont() to MutationObserver ---
  const observer = new MutationObserver(() => {
    applyStylesBasedOnSettings();
    applyCustomTitle();
    applyCustomFont();
    applyGlobalUiFont(); // <-- THIS WAS MISSING
